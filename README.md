...
│   ├── track1.yaml             ← T1 overrides
│   ├── track2.yaml             ← T2 overrides
│   └── track3.yaml             ← T3 overrides
├── src/
│   ├── models/
│   │   ├── backbone.py         ← LocalCNN → DynamicGAT → EEGMamba
│   │   ├── eegmamba_encoder.py ← NativeMambaBlock + EEGMambaEncoder
│   │   ├── dynamic_gat.py      ← dynamic learned adjacency GAT
│   │   ├── heads.py            ← Track1/2/3 heads
│   │   └── neuromm_model.py    ← full Y-architecture + factory
│   ├── data/
│   │   ├── dataset.py          ← NeuroMMDataset, CandidateDataset, CV split
│   │   ├── preprocessing.py    ← per-channel RobustScaler
│   │   └── augmentations.py    ← SpecAugment, GaussianNoise, Mixup
│   ├── training/
│   │   └── losses.py           ← FocalPolyLoss, WeightedCELoss, MultiTaskLoss
│   ├── evaluation/
│   │   └── metrics.py          ← AUPRC, Weighted-F1, OOFCalibrator, Ensemble
│   └── utils/
│       ├── config.py           ← YAML loader with base-merge
│       ├── logging.py          ← logger + WandB
│       └── seed.py             ← full reproducibility
├── scripts/
│   ├── prepare_data.py         ← download + extract + validate + index dataset
│   ├── pretrain.py             ← multi-task backbone pretraining
│   ├── train_track.py          ← CV training for any track
│   └── predict_candidate.py    ← ensemble inference → submission ZIP
├── tests/
│   └── test_all.py             ← full test suite (pytest)
├── annotations/                ← populated by prepare_data.py
│   ├── neuromm2026_train_val.csv     ← official manifest (downloaded)
│   ├── neuromm2026_index.csv         ← + per-backbone availability flags
│   ├── neuromm2026_smoke_10pct.csv   ← 10% patient subset for smoke tests
│   └── validation_report.json        ← shape/missing-file report
├── archives/                   ← downloaded tarballs
├── processed/features/         ← extracted .npy files (eeg/ + video/<backbone>/)
├── candidate/                  ← test-phase inputs (no labels)
